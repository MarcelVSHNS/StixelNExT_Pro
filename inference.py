...
def main():
    testing_data = StixelData(data_dir=config['data_path'], phase='testing_2', mode=config['mode'],
    ...
    for n in range(len(testing_data) - 1):
        # if idx % 11 == 0:
        #    continue
        # block_offset = idx % 198
        # if block_offset >= 109:
        #    continue
        print(f"Idx: {idx}")
        ...
        stxl_img.save(os.path.join("sample_results", "video2", f'frame_{idx:05d}.jpg'))
